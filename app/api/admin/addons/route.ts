...
export async function GET(req: NextRequest) {
  ...
  const sheetUrl = process.env.ADDONS_SHEET_URL;
  if (!sheetUrl) {
    return NextResponse.json({ error: "ADDONS_SHEET_URL not configured", needsSetup: true }, { status: 404 });
  }
  ...
  try {
    ...
    return NextResponse.json({ error: "Failed to fetch add-ons sheet from Google Sheet" }, { status: 500 });
  }
}
