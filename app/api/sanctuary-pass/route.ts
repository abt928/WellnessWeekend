...
import { sanctuaryPassClaims } from "@/lib/schema";
...
export async function GET() {
  try {
    ...
    const [{ claimed }] = await db
      ...
      .from(sanctuaryPassClaims);
    return NextResponse.json({ remaining: TOTAL - Number(claimed), total: TOTAL });
  } catch (error) {
    console.error("Sanctuary pass GET error:", error);
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (remaining <= 0) {
      return NextResponse.json({ error: "All Sanctuary Pass spots have been claimed" }, { status: 409 });
    }

    await db.insert(sanctuaryPassClaims).values({
      name: name.trim(),
    ...
    console.error("Sanctuary pass POST error:", error);
